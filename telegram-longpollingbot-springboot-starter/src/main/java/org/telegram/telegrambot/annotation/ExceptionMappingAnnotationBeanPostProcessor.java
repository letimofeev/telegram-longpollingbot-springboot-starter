    ...
    public void putWithDuplicatesValidation(Class<? extends Exception> exceptionType, Method method, Object target) {
        ...
        if (exceptionMappingOptional.isPresent()) {
            ...
            if (storedExceptionMapping.getTarget() instanceof DefaultExceptionHandler) {
                methodContainer.putExceptionMapping(exceptionType, new MethodTargetPair(method, target));
            } else if (!(target instanceof DefaultExceptionHandler)) {
                String message = String.format("Found duplicate method annotated as @ExceptionMapping with same exception: %s and %s", method, exceptionType);
